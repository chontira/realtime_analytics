...
appid = 'tfidf' # ชื่อแอพของเรา
gearkey = '9DUlJqK6eBJgJp4' # key
gearsecret = 'PG8rNT8BULNoMESLehcVZZlKZ' # secret


client.create(gearkey,gearsecret,appid,{'debugmode': True}) # สร้างข้อมูลสำหรับใช้เชื่อมต่อ
client.setalias("New") # ตั้งชื่้อ
...
client.subscribe("/streams-plaintext-input") # ชื่อช่องทางส่งข้อมูล ต้องมี / นำหน้า และต้องใช้ช่องทางเดียวกันจึงจะรับส่งข้อมูลระหว่างกันได้
...
while i<10:
    ...
    client.publish("/streams-plaintext-input", "hello from Ekarat" )   # ส่งข้อมูลไปให้ทุกคนที่ subscribe test
