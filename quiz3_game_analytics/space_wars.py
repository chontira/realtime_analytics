...
appid = 'gameanalytic' # ชื่อแอพของเรา
gearkey = 'bloTPDjuEYAErXD' # key
gearsecret = 'E9tiN6JBH0TYMbKPwF5stLRm1' # secret
...
class RepeatedTimer(object):    #timer for collect data
...
def show_score(name, score, level, font_size = 16, x=10, y=10):
	...
	name_text = score_font.render("Name : " + name, True, (255, 255, 0))
	screen.blit(level_text, (x, y))
	screen.blit(score_text, (x, y + 5 + font_size))
	screen.blit(name_text, (x, y + 25 + font_size))

def show_realtime_score(font_size = 16, x=700, y=10):
	global current_score
	top_user = sorted(current_score, key=current_score.get, reverse=True)[:3]
	score_font = pygame.font.Font('freesansbold.ttf', font_size)
	for key in top_user:
		score_text = score_font.render(key+":"+str(current_score[key]), True, (255, 255, 0))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score,game_data):
	...
	# Predict player type
	filename = 'saved_model/STD_PCA_Kmean_noa4'
	loaded_model = pickle.load(open(filename, 'rb'))
	result = loaded_model.predict(game_data)
	clustering_result = {0: 'Causal Killer', 1: 'Hardcore Achiver', 2: 'Hardcore Killer', 3: 'Causal Achiver'}
	...
	message_display_center('You are : ' + clustering_result[result[0]], font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY *7/10))
...
def collect_data():
	...
	A0.append(A0_pos_X)
	A1.append(A1_pos_Y)
	A4.append(A4_shots_avg)
	# print(A4_shots_avg)
	# print("Hello!", A0_pos_X, A1_pos_Y, A2_coins, A3_shots_enemies, A4_shots, A5_shots_wo_enemies)
	A4_shots_avg = 0
...
PLAYER_NAME	= 'NEW'		#sws.PLAYER_NAME	# 'DAN'

current_score = {PLAYER_NAME:0}
def callback_message(topic, message):
	global current_score
	name, score = message.replace('b','').strip("'").split(':')
	current_score[name] = int(score)
	print(current_score)

client.create(gearkey,gearsecret,appid,{'debugmode': True}) # สร้างข้อมูลสำหรับใช้เชื่อมต่อ
client.setalias("New") # ตั้งชื่้ออุปกรณ์
...
explosion_image		= [pygame.image.load(os.path.join(images_path, 'explosion_01.png')),
                       ...
                       pygame.image.load(os.path.join(images_path, 'explosion_03.png'))]
...
while not quit_game:
	...
	num_of_coins = 3				# change number of coin here
...
#----------------------------------------------------------------------------------------------------------------
	coin = []
	for i in range(num_of_coins):   # change coin speed in speedY
		coin.append(SpaceCoin(coin_image, explosion_image[2], speedY = 5, hit_points = level))
...
#----------------------------------------------------------------------------------------------------------------
	...
	# Main Game Play Loop
	# --------------------
	rt = RepeatedTimer(1, collect_data)
...
#			score += level_score_increase
		...
		for event in pygame.event.get():	
			...
			if event.type == pygame.KEYDOWN:
				...
				# if space key, fire bullet			
					...
					bullet.fire_bullet(player)		#A4 = create new variable
					A4_shots += 1
					A4_shots_avg += 1
				A0_pos_X = player.posX	#A0
				A1_pos_Y = player.posY	#A1
		...
		if game_over:
			A5_shots_wo_enemies = A4_shots - A3_shots_enemies
			# print(average(A0), average(A1), A2_coins, A3_shots_enemies, A4_shots, average(A4), A5_shots_wo_enemies)
			if isWrite: # write collected data to files
				with open('game_data.csv', "a") as outfile:
					outfile.write(f'{average(A0)},{average(A1)},{A2_coins},{A3_shots_enemies},{A4_shots},{average(A4)},{A5_shots_wo_enemies}\n')
				isWrite = False
				game_data = [[average(A0),average(A1),A2_coins,A3_shots_enemies,average(A4),A5_shots_wo_enemies]]
			player.explosion_counter = 0
			show_game_over(screen_sizeX, screen_sizeY, score, session_high_score,game_data)
			show_score(PLAYER_NAME,score, level)
		else:
#---------------------------------------------------------------------------------------------------
			for i in range(num_of_coins):
				if coin[i].explosion_counter >= 1:
					...
					if is_collision(coin[i], player):   #A2 = Create new var
						explosion_sound.play()
						coin[i].explosion_counter = 10
...
#---------------------------------------------------------------------------------------------------				
			...
			for i in range(num_of_enemies):
				...
				if enemy[i].explosion_counter >= 1:
					...
					elif bullet.state == 'show' and is_collision(enemy[i], bullet) :   #A3 = create new var
						explosion_sound.play()
						enemy[i].explosion_counter = 10
						score += enemy[i].hit_points
						bullet.state = 'hide'
						client.publish("/scores", PLAYER_NAME+": "+str(score) ) # send name and score to netpie
						A3_shots_enemies += 1
